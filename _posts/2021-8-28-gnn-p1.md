...
此外，由于 $\mathbf{L}$ 的每一列加起来都为0，因此其行列式为0，也就至少有一个 0 特征值。
...
$$
\left\| \mathbf{u}_k \right\| _{\mathrm{Dir} }^{2}=\mathbf{u}_k^{\top}\mathrm{L}\mathbf{u}_k=\mathbf{u}_k^{\top}\lambda _i\mathbf{u}_k=\lambda _i
$$
...
> 事实上，当 $\lambda _l=0$ 时，$\mathbf{u}_l$ 在同一连通分支上的值都相等（对应上图中的 $\phi_1$）。
...
> 由此可引出 Laplace 矩阵的另一个性质：$\mathbf{L}$ 的零特征值的重数，等于图中连通分支的个数。
...
$\mathbf{\hat{f} }_{\mathrm{in} }$ 是不同频率基的系数，因此我们可以通过点乘 $\hat{g}\left( \lambda _k \right)$ 来修改 $\lambda _k$​ 频率的系数，一个常见的滤波就是热核函数。
...
[2] Hamilton, William L. “[Graph Representation Learning.](https://www.morganclaypool.com/doi/10.2200/S01045ED1V01Y202009AIM046)” *Synthesis Lectures on Artificial Intelligence and Machine Learning*.



[3] Bougleux, Sébastien, et al. “[Discrete Regularization on Weighted Graphs for Image and Mesh Filtering.](https://hal.archives-ouvertes.fr/hal-00333374)” *1st International Conference on Scale Space and Variational Methods in Computer Vision (SSVM 2007)*.
...
这里的 $a_i$ 是中间红色区域的面积。可以看到，这个定义和图非常相似，就是把权重定义设成了 $w_{ij}=\frac{1}{2}\left( \cot \alpha _{i,j}+\cot \beta _{i,j} \right) $​。
...
如果想进一步理解的话，可以参看论文《[Geometric deep learning: going beyond Euclidean data](https://doi.org/10.1109/MSP.2017.2693418 )》，文中定义了梯度算子和散度算子，并可以验证这样的梯度算子和散度算子是伴随算子，于是可以认为这样的定义是比较合适的，然后就能推导出图 Laplace 算子。

![image-20210828214334917](/zjblog/assets/images/2021-8-28-gnn-p1/image-20210828214334917.png)
...
![image-20210828214420678](/zjblog/assets/images/2021-8-28-gnn-p1/image-20210828214420678.png)
