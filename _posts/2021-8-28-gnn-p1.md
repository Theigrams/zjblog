...
|              | 欧式空间                                                     | 图傅里叶                                                     |
| ------------ | ------------------------------------------------------------ | ------------------------------------------------------------ |
| 特征方程     | $\Delta \mathrm{e}^{-\mathrm{i}kx}=-k^2\mathrm{e}^{-\mathrm{i}kx}$ | $\mathbf{Lu}_k=\lambda _k\mathbf{u}_k$                       |
...
可以看到，这个定义和图非常相似，就是把权重定义设成了 $w_{ij}=\frac{1}{2}\left( \cot \alpha _{i,j}+\cot \beta _{i,j} \right) $​。
